...
namespace DVLD_DataAccessLayer
{
    public class clsLicenseInfoData
    {
        ...
        static public DataTable GetLicenseDriverDataUsingLicenseID(int LicenseID)
        {
            SqlConnection connection = new SqlConnection(clsDataAccessSettings.ConnectionString);
            string query = @"SELECT        LicenseClasses.ClassName, CONCAT (People.FirstName , ' ' , People.SecondName , ' ' , People.ThirdName , ' ' , People.LastName) AS FullName, People.ImagePath , Licenses.LicenseID, People.NationalNo, Case 
                            When People.Gendor = 0 Then 'Male'
                            When People.Gendor = 1 Then 'Female'
                            Else 'Unkown'
                            End As Gendor,
        ...
        static public bool CheckIfLicenseExist(int LicenseID)
        {
            SqlConnection connection = new SqlConnection(clsDataAccessSettings.ConnectionString);
            string query = @"SELECT CASE 
                         WHEN EXISTS
                         (SELECT 1 FROM Licenses WHERE LicenseID = @LicenseID)
                         THEN 1
                         ELSE 0
                         END";
            SqlCommand command = new SqlCommand(query, connection);
